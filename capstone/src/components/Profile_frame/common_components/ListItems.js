...
export const ListItems = (props) => {
  ...
  let buttonsName = props.type_content
    ? `My ${props.type_content} (${libraryArr.filter((n) => n.info.category_type == props.type_content).length})`
    : "Files " + libraryArr.length;

  let carouselMode = <CarouselShow {...props} />;
  let cards = (
    <ElemShow cardSetOn={cardSetOn} openViewer={openViewer} {...props} />
  );
