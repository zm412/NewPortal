...
const ListTypeContent = ({ arr, type_content }) => {
  ...
  return <option value={title}>{type_content} </option>;
};
...
export const UploadFile = () => {
  ...
  const [typeC, setTypeC] = React.useState("Select here");
  ...
  let handleChange = (e) => {
    ...
    document
      .querySelector(e.target.dataset.connect)
  ...
  let closeUpload = () => {
    ["#id_type_category", "#id_name_category", "#id_form"].map((n) =>
      document.querySelector(n).classList.add("none_display"),
    );
  ...
  const getExt = (typeC, categoriesArr) => {
    if (categoriesArr) {
      let ext = categoriesArr.find(
        (n) => n.type_content == typeC,
      ).valid_extensions;
      return ext;
    }
    return "";
  ...
  let types_content = categories
    ? categories
        .filter((item) => item.type_content == "null")
        .reduce((newArr, item) => {
  ...
  let list = types_content.map((n, i) => (
    <ListTypeContent key={i} arr={categories} type_content={n} />
  ));
  let list2 = categories
    .filter((item) => item.type_content == typeC)
    .map((item, i) => (
    ...
    <div className="row mt-5">
      ...
      <div id="id_type_category" className="none_display">
        ...
        <div id="id_name_category" className="none_display">
          <h6>Select name category</h6>
          <select
            className="form-select"
            id="file_up"
            data-connect="#id_form"
            multiple={true}
            onChange={chooseName}
          >
            <option disabled>Select</option>
