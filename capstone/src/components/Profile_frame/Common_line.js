...
const FilterBlock = (props) => {
  ...
  return (
    <div>
      <button
        className="btn-round btn-yellow mt-2 p-2 center_cl col-sm-4"
        onClick={(e) =>
          document
            .querySelector("#id_name_cat")
...
export const Common_line = () => {
  ...
  let links = [1, 3, 5].map((n, i) => (
    <a href="#" key={i} onClick={onClickLink} className="no_decoration">
      <strong data-limit={n}> {n} </strong>
    </a>
  ));

  let str_filters =
    filterArr && filterArr.length > 0
      ? filterArr.reduce((str, n, i) => {
  ...
  return (
    <div className="m-4">
      ...
      {isThereFilter && (
        <div>
          Choosen categories: <strong>{str_filters}</strong>
        </div>
      )}
      Posts per page: {links}
      <p className="center_cl">
        Page <strong>{counter}</strong>
      </p>
      <Common_list />
      {quantity > limit_num && counter > 1 && (
        <Button onClick={button_prev} variant="success" className="m-3">
          Prev
        </Button>
      )}
